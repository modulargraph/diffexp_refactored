...
#include "inplace_horner_chart.hpp"
#include <iomanip>
int main(int argc,char** argv) {try {
 require(argc==4,"usage: benchmark_charts METHOD DENSE REPEATS");
 const std::string method=argv[1];const bool dense=std::stoi(argv[2]);const unsigned repeats=std::stoul(argv[3]);
 ...
 auto run=[&] {
  ...
  if(method=="inplace_horner")return adjoint_detail::inplace_horner_chart(compiled,initial,center,step,order);
