#include "diffexp/kernel/singular_indicial.hpp"
...
PreparedRecurrenceOperator<Rational> prepared_operator(
  ...
  prepared.nhat_lags.front().valuations.assign(
      9, diffexp::kernel::kCompleteInfinity);
...
int main() {
  ...
  const auto indicial =
      diffexp::kernel::certify_exact_affine_jordan_operator(prepared);
  ...
  diffexp::kernel::ExactJordanIndicialCertificate reachability;
  reachability.blocks = {
  ...
  check("finite log ceiling includes the nmax resonance but not nmax+1",
        diffexp::kernel::singular_indicial_detail::exact_reachable_jordan_log_ceiling(
  ...
  const auto resonant = diffexp::kernel::certify_exact_affine_jordan_schedule(
  ...
  const auto pseudo = diffexp::kernel::certify_exact_affine_jordan_schedule(
  ...
  check("malformed prepared Jordan superdiagonal is rejected exactly",
        rejects([] {
          (void)diffexp::kernel::certify_exact_affine_jordan_operator(
  ...
  check("submitted schedule offsets cannot override the retained root",
        rejects([&] {
          (void)diffexp::kernel::certify_exact_affine_jordan_schedule(
