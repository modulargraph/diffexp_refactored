...
using diffexp2::EpsilonMatrix;
using diffexp2::EpsilonWindow;
using diffexp2::EvaluationOptions;
...
void test_direct_value_theta_and_residual() {
  ...
  const auto certificate = diffexp2::certify_theta_residual(
...
void test_epsilon_exponent_and_log_shift() {
  ...
  solution.sectors[0].coefficients = {ComplexBall(2), ComplexBall(0),
                                      ComplexBall(0)};
  const auto log_evaluation = diffexp2::evaluate_local_solution(
      solution, RealEvaluationPoint::rational("1/2"));
...
void test_loud_branch_failures() {
  ...
  try {
    (void)diffexp2::evaluate_local_solution(
