...
using diffexp2::RecurrenceResult;
...
void test_transaction_local_candidate() {
  ...
  recurrence.validity = {-1, diffexp2::kCompleteInfinity};
  recurrence.top_valid = -1;
  const auto original_validity = recurrence.validity;

  bool ordinary_rejected = false;
  try {
    (void)diffexp2::assemble_recurrence(prepared, problem, recurrence);
  ...
  auto candidate = diffexp2::assemble_scc_recurrence_candidate(
  ...
  auto acb_candidate = diffexp2::assemble_scc_recurrence_candidate(
  ...
  try {
    diffexp2::require_exact_domain_for_deferred_scc_candidate(acb_candidate);
  } catch (const RecurrenceError& error) {
