...
bool canonical_json_dag_container_roundtrip(
  ...
  diffexp::kernel::checkpoint::write_atomic(path.string(), header, payload);
  const auto decoded = diffexp::kernel::checkpoint::read_json(path.string());
  const auto text = diffexp::kernel::checkpoint::read(path.string());
  write_container_fixture(legacy_path, 1, text.header_json,
                          text.payload_json);
  const auto legacy = diffexp::kernel::checkpoint::read_json(legacy_path.string());
  ...
  try {
    (void)diffexp::kernel::checkpoint::read_json(unused_path.string());
...
bool streamed_large_container_roundtrip(
  ...
  const auto decoded =
      diffexp::kernel::checkpoint::read_json(path.string());
  const bool ok =
