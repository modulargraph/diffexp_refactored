...
// Serialize construction of recursively derived immutable values. A builder
// may request another key, while the in-flight set turns a dependency cycle
// into a deterministic error instead of a promise deadlock.
...
class ImmutableRecursiveCache {
 public:
  ...
  Lookup get_or_build(const Key& key, const std::string& contract,
    ...
    if (!inserted)
      throw std::logic_error("immutable recursive cache insertion failed");
    ++builds_;
