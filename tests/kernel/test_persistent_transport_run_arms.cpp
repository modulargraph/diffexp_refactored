...
diffexp::kernel::ComplexBall decode_encoded_ball(const json::value& raw) {
  ...
  auto ball = diffexp::kernel::ComplexBall::from_strings(
...
json::object value_solver(const std::string& center,
  ...
  return json::object{
      {"schema", "diffexp3-native-regular-value-solver-prototype-v1"},
      {"run", json::object{
           {"nmax", nmax}, {"p", 0}, {"has_initial", true},
           {"adaptive_probe", false}, {"a_target", "0"},
           {"b_target", "0"}, {"a_shift_min", 0},
...
json::object physical_value_solver(
    ...
    const std::string& relative_error_max = "1/100") {
  return json::object{
      {"schema", "diffexp3-native-ordinary-physical-value-solver-v1"},
...
json::value contract_and_export(const std::string& session,
  ...
  const auto contracted = request(json::object{
      ...
      {"checkpoint_policy", json::object{
           {"schema",
            "diffexp3-deterministic-transport-contraction-checkpoints-v1"},
           {"root", root}}},
...
void test_regular_value_hop_checkpoint() {
  const std::string checkpoint =
      "/tmp/diffexp3-regular-value-hop-roundtrip.de2cp";
  const std::string tampered_checkpoint =
      "/tmp/diffexp3-regular-value-hop-retained-value-tampered.de2cp";
...
void test_streaming_consumed_transport() {
  const std::string checkpoint =
      "/tmp/diffexp3-streaming-consumed-roundtrip.de2cp";
  std::remove(checkpoint.c_str());
  std::string session;
  std::string restored;
  try {
    ...
    const auto& lower_match_reference =
        lower.at("match_reference").as_object();
    if (lower.at("capability") != "consuming-transport-hop-v2" ||
        lower_match_reference.at("schema") !=
            "diffexp3-bounded-consuming-hop-reference-v1" ||
        !lower_match_reference.at("exact_lineage_retained").as_bool() ||
        lower_match_reference.if_contains("provenance_identity") != nullptr ||
        lower_match_reference.if_contains("planned_hop") != nullptr ||
        lower_match_reference.if_contains("sealed_local_lineage") != nullptr ||
        lower_match_reference.at("provenance_reference")
                .as_object().at("algorithm") != "fnv1a64-v1" ||
        lower_match_reference.at("planned_hop_reference")
                .as_object().at("algorithm") != "fnv1a64-v1" ||
        lower_match_reference.at("sealed_local_lineage_reference")
                .as_object().at("algorithm") != "fnv1a64-v1")
      throw std::runtime_error(
          "streaming hop echoed an unbounded exact lineage: " +
          json::serialize(lower_match_reference));
...
void test_consuming_transport() {
  const std::string checkpoint =
      "/tmp/diffexp3-consuming-transport-roundtrip.de2cp";
  std::remove(checkpoint.c_str());
  std::string session;
  std::string restored;
  try {
...
void test_frame_independent_physical_value_wiring() {
  const std::string checkpoint =
      "/tmp/diffexp3-frame-independent-physical-hop.de2cp";
  const std::string resaved_checkpoint =
      "/tmp/diffexp3-frame-independent-physical-hop-resaved.de2cp";
  const std::string private_prefix_checkpoint =
      "/tmp/diffexp3-frame-independent-private-prefix.de2cp";
  const std::string overlap_checkpoint =
      "/tmp/diffexp3-frame-independent-overlap-recenter.de2cp";
  std::remove(checkpoint.c_str());
...
void test_acb_terminal_factorized_consumed_checkpoint() {
  const std::string checkpoint =
      "/tmp/diffexp3-terminal-factorized-consumed.de2cp";
  const std::string corrupted =
      "/tmp/diffexp3-terminal-factorized-consumed-corrupt.de2cp";
  const std::string prepublication_checkpoint =
      "/tmp/diffexp3-terminal-factorized-prepublication.de2cp";
  std::remove(checkpoint.c_str());
  ...
  try {
    ...
    const auto compact_basis_0 = solve_multiblock_local(
        session, lower_chart, "-2/3",
        "terminal-factorized-compact-basis-0", {"1", "1"}, 8);
    const auto compact_basis_1 = solve_multiblock_local(
        session, lower_chart, "-2/3",
        "terminal-factorized-compact-basis-1", {"0", "1"}, 8);
    const auto compact_hop = consume_basis_hop(
        session, plan, plan_checkpoint, "lower", anchor,
        anchor_checkpoint, {compact_basis_0, compact_basis_1},
        "terminal-factorized-compact");
    require_ok(compact_hop,
               "terminal factorized compaction consume_hop");
    const auto compact_local = compact_hop.at("next_local").as_object();
    const auto before_compaction = request(json::object{
        {"schema", 2}, {"op", "session.counters"},
        {"session", session}});
    require_ok(before_compaction,
               "terminal factorized pre-compaction counters");
    const auto compacted = request(json::object{
        {"schema", 2},
        {"op", "local.release_terminal_factorized_owner"},
        {"session", session},
        {"local", compact_local.at("local")},
        {"local_checkpoint_identity",
         compact_local.at("checkpoint_identity")}});
    require_ok(compacted,
               "terminal factorized rolling-local compaction");
    const auto after_compaction = request(json::object{
        {"schema", 2}, {"op", "session.counters"},
        {"session", session}});
    require_ok(after_compaction,
               "terminal factorized post-compaction counters");
    if (compacted.at("released_terminal_factorized_owner") != true ||
        compacted.at("coefficient_retention") !=
            "terminal-local-only" ||
        counter(after_compaction,
                "process_local_coefficients_live") >=
            counter(before_compaction,
                    "process_local_coefficients_live"))
      throw std::runtime_error(
          "rolling terminal compaction did not release its hidden basis-local coefficient closure: " +
          json::serialize(compacted) + " / " +
          json::serialize(before_compaction) + " / " +
          json::serialize(after_compaction));
    const auto duplicate_compaction = request(json::object{
        {"schema", 2},
        {"op", "local.release_terminal_factorized_owner"},
        {"session", session},
        {"local", compact_local.at("local")},
        {"local_checkpoint_identity",
         compact_local.at("checkpoint_identity")}});
    if (duplicate_compaction.at("status") != "error")
      throw std::runtime_error(
          "rolling terminal compaction accepted a duplicate owner release");
    ...
    const auto short_pair = request(json::object{
        ...
        {"checkpoint_policy", json::object{
             {"schema",
              "diffexp3-deterministic-transport-pair-contraction-checkpoints-v1"},
    ...
    if (strict_conditioning.at("schema") !=
            "diffexp3-transport-pair-conditioning-diagnostics-v1" ||
    ...
    const auto require_small_zero = [](const json::value& raw,
      ...
      for (const auto& coefficient :
        ...
        if (!ball.contains_zero() ||
            diffexp::kernel::Magnitude::upper_abs(ball)
    ...
    if (setenv("DIFFEXP_DIAGNOSTIC_TERMINAL_CONTRACTION_ROUTE",
               "factorized", 1) != 0)
      throw std::runtime_error(
    ...
    try {
      ...
      unsetenv("DIFFEXP_DIAGNOSTIC_TERMINAL_CONTRACTION_ROUTE");
      throw;
    }
    unsetenv("DIFFEXP_DIAGNOSTIC_TERMINAL_CONTRACTION_ROUTE");
    require_small_zero(
    ...
    if (setenv("DIFFEXP_DIAGNOSTIC_TERMINAL_COMPOSED_ADJOINT",
               "require", 1) != 0) {
      unsetenv("DIFFEXP_DIAGNOSTIC_TERMINAL_CONTRACTION_ROUTE");
      throw std::runtime_error(
    ...
    try {
      ...
      unsetenv("DIFFEXP_DIAGNOSTIC_TERMINAL_CONTRACTION_ROUTE");
      unsetenv("DIFFEXP_DIAGNOSTIC_TERMINAL_COMPOSED_ADJOINT");
      throw;
    }
    unsetenv("DIFFEXP_DIAGNOSTIC_TERMINAL_CONTRACTION_ROUTE");
    unsetenv("DIFFEXP_DIAGNOSTIC_TERMINAL_COMPOSED_ADJOINT");
    ...
    for (const auto& coefficient :
      ...
      if (!value.contains_zero() ||
          diffexp::kernel::Magnitude::upper_abs(value).approximate_upper() >
              1e-2)
        throw std::runtime_error(
...
int main() {
  const std::string first_checkpoint =
      "/tmp/diffexp3-transport-run-arms-first.de2cp";
  const std::string second_checkpoint =
      "/tmp/diffexp3-transport-run-arms-second.de2cp";
