...
json::object run_session_command(const json::object& root) {
  ...
  if (operation == "local.specialize_rational_shadow") {
    const auto* exact_shadow_identity =
        root.if_contains("rational_shadow_identity");
    const auto* shadow_identity_reference =
        root.if_contains("rational_shadow_identity_reference");
    if ((exact_shadow_identity == nullptr) ==
        (shadow_identity_reference == nullptr))
      throw std::invalid_argument(
          "Rational-shadow specialization requires exactly one exact identity or compact identity reference");
    if (exact_shadow_identity != nullptr)
      require_exact_keys(root,
      ...
      require_exact_keys(root,
          {"schema", "op", "session", "source_session", "source_local",
           "source_checkpoint_identity", "target_scc",
           "rational_shadow_identity_reference", "checkpoint_identity"},
          "local.specialize_rational_shadow request");
    ...
    try {
      const auto shadow_binding_matches = [&](const std::string& identity) {
        if (exact_shadow_identity != nullptr)
          return identity == required_string(
              root, "rational_shadow_identity");
        return compact_matching_identity_reference_matches(
            *shadow_identity_reference, identity,
            "Rational-shadow identity reference");
      };
      if (!shadow_binding_matches(source_owner->rational_shadow_identity()) ||
          !shadow_binding_matches(target->rational_shadow_identity()) ||
      ...
      json::object column_record{
          {"schema", "diffexp3-native-scc-acb-rational-shadow-column-v1"},
          {"scc_exact_identity_reference",
           compact_matching_identity_reference(target->exact_identity())},
  ...
  if (operation == "tile.plan") {
    ...
    auto result = plan->publication_summary();
    result["status"] = "ok";
