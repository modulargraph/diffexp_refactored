...
LocalSolution<Rational> exponential_local(const char *a, std::uint32_t order) {
  ...
  sector.a = diffexp2::ExactScalarDescriptor::rational(a);
...
int main() {
  try {
    ...
    const auto point = diffexp2::RealEvaluationPoint::rational("1/4");
    const auto certified =
        diffexp2::evaluate_singular_rational_shadow_with_certified_tail(
    ...
    const auto inflated =
        diffexp2::inflate_certified_singular_seed_value(certified);
    const auto quarter = ComplexBall::from_strings("1/4");
    const auto exact = quarter * diffexp2::local_detail::cb_exp(quarter);
    ...
    const auto framed_bridge = diffexp2::
        certify_singular_rational_shadow_ordinary_bridge_with_seed_map(
            ...
            [](const diffexp2::EpsilonVector &seed,
               const diffexp2::RealEvaluationPoint &seed_point)
            ...
            EpsilonWindow{0, 0}, 4,
            diffexp2::RealEvaluationPoint::rational("1/5"), "1/2", point, 16,
            "3/50", {}, &tail_prefix);
    ...
    const auto negative_bridge =
        diffexp2::certify_singular_rational_shadow_ordinary_bridge(
            equation, local, EpsilonWindow{0, 0}, 4,
            diffexp2::RealEvaluationPoint::rational("-1/6"), "1/2",
            diffexp2::RealEvaluationPoint::rational("-1/4"), 16, "1/8");
    ...
    std::fill(structurally_zero_local.sectors.front().coefficients.begin(),
              ...
              Rational(0));
    const auto negative = diffexp2::prepare_singular_rational_shadow_tail_model(
    ...
    jordan.prescriptions.push_back(diffexp2::Prescription{"t", 1, 1, 1});
    ...
    if (jordan_model.status != TailMajorantStatus::Certified ||
      ...
      throw std::runtime_error(
          ...
          jordan_model.detail);
    diffexp2::EvaluationOptions lower_rim;
    lower_rim.imaginary_sign = -1;
    const auto jordan_seed =
        diffexp2::evaluate_singular_rational_shadow_with_certified_tail(
            *jordan_model.model, jordan,
            diffexp2::RealEvaluationPoint::rational("-1/4"), lower_rim);
    ...
    const auto jordan_value =
        diffexp2::inflate_certified_singular_seed_value(jordan_seed);
    auto expected_log =
        diffexp2::local_detail::cb_log(ComplexBall::from_strings("1/4"));
    expected_log += diffexp2::local_detail::imaginary_pi(-1);
    ...
    const auto nonnormal_future = diffexp2::singular_tail_majorant_detail::
    ...
    const auto resonant_future = diffexp2::singular_tail_majorant_detail::
    ...
    const auto nilpotent_future = diffexp2::singular_tail_majorant_detail::
    ...
    const auto unweighted_transfer = diffexp2::singular_tail_majorant_detail::
