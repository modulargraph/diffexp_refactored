...
bool finite_laurent_pole_materialization() {
  diffexp::kernel::LocalSolution<diffexp::kernel::Rational> column;
  column.chart.center_exact = "0";
  column.chart.scale_exact = "1";
  column.chart.radius = diffexp::kernel::ComplexBall(2);
  column.epsilon = {0, 2};
  column.taylor_complete_max = 0;
  column.dimension = 1;
  column.checkpoint_identity = "pole-basis";
  diffexp::kernel::LocalSector<diffexp::kernel::Rational> sector;
  sector.a = diffexp::kernel::ExactScalarDescriptor::rational("0");
  sector.b = diffexp::kernel::ExactScalarDescriptor::rational("0");
  sector.coefficients = {
      diffexp::kernel::Rational(0), diffexp::kernel::Rational(1),
      diffexp::kernel::Rational(0)};
  column.sectors.push_back(std::move(sector));
  const diffexp::kernel::EpsilonFrame<diffexp::kernel::Rational> pole_weight(
      {-1, 1}, {diffexp::kernel::Rational(2), diffexp::kernel::Rational(0),
                diffexp::kernel::Rational(0)});
  const auto materialized = diffexp::kernel::materialize_local_basis_weights(
      std::vector<const diffexp::kernel::LocalSolution<diffexp::kernel::Rational>*>{
          &column},
      diffexp::kernel::FiniteLaurentVector<diffexp::kernel::Rational>{pole_weight},
      "pole-materialized");
  ...
  return materialized.epsilon.min_power == -1 &&
      ...
      coefficients[1] == diffexp::kernel::Rational(2) &&
...
bool run_domain(const std::string& domain) {
  ...
  const auto base = std::filesystem::temp_directory_path() /
      ("diffexp3-plan-match-hop-" + domain + "-" +
  ...
  const auto container = diffexp::kernel::checkpoint::read(path);
  ...
  const auto expected_native_schema = domain == "rational"
      ...
      : "diffexp3-retained-acb-match-v6";
  ...
  if (domain == "rational") {
    corrupt_native["schema"] = "diffexp3-retained-unknown-match-v9";
  ...
  const auto resaved_payload = json::parse(
      diffexp::kernel::checkpoint::read(resaved_path).payload_json).as_object();
