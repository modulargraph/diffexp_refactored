...
// Evaluate the single-valued part of an exact Rational local solution without
// first specializing its coefficients to balls.  This is deliberately
// narrower than evaluate_local_solution: it only accepts integer t powers,
// b == 0, and log power zero.  In that regular case evaluation commutes with
// every t-independent exact Laurent right transformation.  Keeping both
// operations in Rational arithmetic is important for confluent fundamental
// columns whose small nonzero determinant is otherwise lost while summing
// large Taylor terms.
inline std::optional<FiniteLaurentVector<Rational>>
evaluate_regular_rational_local_solution_exact(
    const LocalSolution<Rational>& solution,
    const RealEvaluationPoint& point, const EvaluationOptions& options,
    EpsilonWindow output_window) {
  validate_local_solution(solution, false);
  if (point.exact_coordinate.empty()) return std::nullopt;

  Rational exact_t;
  try {
    exact_t = Rational(point.exact_coordinate);
  } catch (const std::invalid_argument&) {
    return std::nullopt;
  }
  if (exact_t.sign() != point.sign)
    throw std::invalid_argument(
        "exact Rational evaluation point disagrees with its retained sign");
  if (output_window.complete_max > solution.epsilon.complete_max)
    throw MatchingArithmeticError(
        MatchingArithmeticErrorCode::InsufficientCompleteWindow,
        "exact Rational local evaluation does not cover the requested "
        "epsilon window", std::nullopt, std::nullopt,
  ...
  if (options.t_order_reduction >= full_taylor_width)
    throw std::invalid_argument(
        "t-order reduction removes every exact Rational coefficient");
  const auto taylor_width =
      full_taylor_width - options.t_order_reduction;

  std::vector<Rational> t_powers(taylor_width, Rational(1));
  for (std::size_t n = 1; n < taylor_width; ++n)
    t_powers[n] = t_powers[n - 1] * exact_t;

  const auto integer_power = [&](std::int64_t exponent)
      -> std::optional<Rational> {
    if (exact_t.is_zero() && exponent < 0) return std::nullopt;
    ...
    if (exponent < 0) value = Rational(1) / value;
    return value;
  };

  std::vector<Rational> evaluated(
      solution.epsilon.width() * solution.dimension, Rational(0));
  for (const auto& sector : solution.sectors) {
    if (!local_detail::material_sector(sector)) continue;
    ...
    try {
      std::size_t parsed = 0;
      exponent = std::stoll(sector.a.canonical, &parsed, 10);
      if (parsed != sector.a.canonical.size()) return std::nullopt;
    } catch (const std::exception&) {
      return std::nullopt;
    }
    const auto outer = integer_power(exponent);
    ...
    for (std::size_t epsilon_index = 0;
      ...
      for (std::uint32_t component = 0;
        ...
        for (std::size_t n = 0; n < taylor_width; ++n)
          sum += sector.coefficients[local_detail::sector_index(
                     solution, epsilon_index, n, component)] *
                 t_powers[n];
        evaluated[epsilon_index * solution.dimension + component] +=
            *outer * sum;
      }
    }
  }

  // A caller may select a common lower edge above this column's stored edge
  // only when the discarded exact coefficients are structural zero.
  for (std::int64_t power = solution.epsilon.min_power;
       power < output_window.min_power; ++power) {
    const auto epsilon_index = static_cast<std::size_t>(
        power - solution.epsilon.min_power);
    for (std::uint32_t component = 0;
         component < solution.dimension; ++component)
      if (!evaluated[epsilon_index * solution.dimension + component]
               .is_zero())
        throw MatchingArithmeticError(
            MatchingArithmeticErrorCode::InsufficientCompleteWindow,
            "exact Rational local evaluation would discard a nonzero lower "
            "epsilon coefficient", component, std::nullopt,
            static_cast<std::int32_t>(power));
  }

  FiniteLaurentVector<Rational> result;
  result.reserve(solution.dimension);
  for (std::uint32_t component = 0; component < solution.dimension;
       ++component) {
    std::vector<Rational> coefficients;
    coefficients.reserve(output_window.width());
    for (std::int64_t power = output_window.min_power;
         power <= output_window.complete_max; ++power) {
      if (power < solution.epsilon.min_power) {
    ...
    result.emplace_back(output_window, std::move(coefficients));
  }
  return result;
}
...
inline RefinedAcbLaurentMatch refine_acb_finite_laurent_match(
  ...
  if (exact_formal_negative_coefficients_are_zero) {
    try {
      ...
      // Formal-module saturation proves that the transformed local columns
      // have no negative epsilon support.  Point evaluation is a different
      // linear map: distinct t/log rows can coalesce at the overlap, so the
      // evaluated epsilon^0 matrix may lose rank even though the complete
      // nonnegative Laurent matrix remains invertible.  More Acb precision
      // cannot repair that structural rank loss.  In that case retain the
      // exact nonnegative basis and use the already-certified Laurent pivot
      // path, which is allowed to select a positive-epsilon pivot.
      const auto detail = std::string(error.what());
      const bool singular_epsilon_zero =
          error.code ==
              MatchingArithmeticErrorCode::SingularOrIncompleteSystem &&
          detail.find("certified epsilon-zero factorization") !=
              std::string::npos &&
          detail.find("no proved nonzero pivot remains") !=
              std::string::npos;
      if (!singular_epsilon_zero) throw;
      laurent_factorization =
          factor_preconditioned_acb_finite_laurent_system(
              candidate_basis,
              context +
                  ": certified Laurent fallback after singular epsilon-zero frame");
      laurent_factorization->preconditioner_kind =
          "laurent-after-singular-epsilon-zero/" +
          laurent_factorization->preconditioner_kind;
    }
  } else
    laurent_factorization =
