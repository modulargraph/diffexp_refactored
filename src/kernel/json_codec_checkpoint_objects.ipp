...
std::shared_ptr<StoredTilePlan> restore_checkpoint_tile_plan_record(
  ...
  if (required_string(object, "schema") !=
      "diffexp3-retained-tile-plan-v2")
    throw std::invalid_argument(
...
restore_checkpoint_transport_endpoint_record_from_terminal(
  ...
  if (bounded_v2)
    require_exact_keys(
        object,
        {"schema", "handle", "checkpoint_identity", "provenance_identity",
         "source", "prepared_row", "approach_direction", "derived_rim",
         "cancellation_mode", "analytic_metadata", "result", "elapsed_ms",
         "runtime_stats"},
        "checkpoint bounded transport endpoint");
  else
    require_exact_keys(
        object,
        {"schema", "handle", "checkpoint_identity", "provenance_identity",
  ...
  auto expected_source = bounded_v2
      ? bounded_transport_endpoint_source(
            binding, plan, local, state, rolling)
      : rolling
  ...
  const auto& prepared_row = bounded_v2
      ? as_object(object.at("prepared_row"),
                  "checkpoint bounded transport endpoint prepared row")
      : as_object(row_record.at("prepared_row"),
                  "checkpoint transport endpoint prepared row");
  if (bounded_v2) {
    require_exact_keys(
        row_record,
        {"tile", "exact_identity_reference", "entry_facts_reference",
         "entry_count"},
        "checkpoint bounded transport endpoint row record");
    if (row_record != compact_prepared_row_provenance_record(0, prepared_row))
      throw std::invalid_argument(
          "checkpoint bounded transport endpoint row reference is stale");
  } else {
  ...
  auto endpoint = std::make_shared<StoredEndpointResult>(
      ...
      bounded_v2 ? std::optional<json::object>(prepared_row) : std::nullopt);
...
restore_checkpoint_planned_match_hop_record(
  ...
  if (required_string(object, "schema") !=
      "diffexp3-retained-planned-match-hop-v2")
    throw std::invalid_argument(
  ...
  const bool compact_handoff =
      handoff_schema == "diffexp3-retained-exact-plan-match-hop-v3";
  if (compact_handoff)
    require_exact_keys(
        handoff,
  ...
  if ((!compact_handoff &&
       handoff_schema != "diffexp3-retained-exact-plan-match-hop-v1") ||
  ...
  if (native_schema == "diffexp3-retained-exact-rational-match-v2") {
    native_match = restore_checkpoint_exact_match_record(
        native_record, basis, incoming);
  } else if (native_schema == "diffexp3-retained-acb-match-v2" ||
             native_schema == "diffexp3-retained-acb-match-v3" ||
             native_schema == "diffexp3-retained-acb-match-v4" ||
             native_schema == "diffexp3-retained-acb-match-v5" ||
             native_schema == "diffexp3-retained-acb-match-v6") {
...
restore_checkpoint_transport_arm_state_record(
  ...
  const bool terminal_factorized_certificate =
      checkpoint_schema == "diffexp3-retained-transport-arm-state-v6";
  const bool consumed_certificate_only =
      terminal_factorized_certificate ||
      checkpoint_schema == "diffexp3-retained-transport-arm-state-v5";
  const bool consumed_compact =
      checkpoint_schema == "diffexp3-retained-transport-arm-state-v4";
  const bool compact_provenance =
      consumed_certificate_only || consumed_compact ||
      checkpoint_schema == "diffexp3-retained-transport-arm-state-v3";
    ...
    for (std::size_t tile = 0; tile < chain.size(); ++tile) {
      ...
      if (tile == 0) {
        ...
        if (certificate_schema ==
          ...
          if (certificate_schema !=
                  "diffexp3-consumed-plan-match-certificate-v1" ||
              required_string(certificate,
...
std::shared_ptr<StoredLineResult> restore_checkpoint_line_result_record(
  ...
  if (required_string(object, "schema") !=
      "diffexp3-retained-line-result-v2")
    throw std::invalid_argument(
  ...
  json::object legacy_provenance{
      {"schema",
       "diffexp3-retained-native-stored-truncation-physical-tile-integral-v1"},
      {"checkpoint_identity", checkpoint_identity},
...
std::shared_ptr<StoredLineResult> restore_checkpoint_line_aggregate_record(
  ...
  const bool compact_single_transport =
      schema == "diffexp3-retained-transport-observable-line-v1" ||
      schema == "diffexp3-retained-transport-observable-line-v2";
  ...
  const bool bounded_aggregate =
      schema == "diffexp3-retained-line-aggregate-v2";
  const bool compact_v2 =
      schema == "diffexp3-retained-transport-observable-line-v2" ||
      schema == "diffexp3-retained-transport-pair-observable-line-v2";
  if (!compact_transport && !bounded_aggregate &&
      schema != "diffexp3-retained-line-aggregate-v1")
    throw std::invalid_argument(
  ...
  const auto validate_observable_rows = [&](
    ...
    for (std::size_t index = 0; index < rows.size(); ++index) {
      ...
      const bool bounded_reference =
          row.if_contains("exact_identity_reference") != nullptr;
      ...
      if (legacy_full_row) {
        ...
        if (exact_identity.empty())
          throw std::invalid_argument(
              std::string(label) + " has an empty exact identity");
        ...
        const auto validate_reference = [&](const json::value& raw_reference) {
          const auto& reference = as_object(raw_reference, label);
          require_exact_keys(
              reference, {"algorithm", "fingerprint", "identity_bytes"},
              label);
          if (required_string(reference, "algorithm") != "fnv1a64-v1" ||
              required_string(reference, "fingerprint").empty() ||
              as_u64(reference.at("identity_bytes"), label) == 0)
            throw std::invalid_argument(
                std::string(label) + " has a malformed content reference");
        };
        validate_reference(row.at("exact_identity_reference"));
        validate_reference(row.at("entry_facts_reference"));
        (void)as_u64(row.at("entry_count"), label);
      } else {
        const auto exact_identity = required_string(row, "exact_identity");
        if (exact_identity.empty())
          throw std::invalid_argument(
  ...
  if (compact_single_transport) {
    ...
    for (std::size_t index = 0; index < components.size(); ++index) {
      ...
      if (bounded_aggregate)
        require_exact_keys(
            component,
            {"index", "sign", "checkpoint_identity",
             "provenance_reference"},
            "checkpoint bounded line aggregate component");
      else
        require_exact_keys(
            component,
      ...
      if (bounded_aggregate) {
        const auto& reference = as_object(
            component.at("provenance_reference"),
            "checkpoint bounded line component provenance reference");
        require_exact_keys(
            reference, {"algorithm", "fingerprint", "identity_bytes"},
            "checkpoint bounded line component provenance reference");
        if (required_string(reference, "algorithm") != "fnv1a64-v1" ||
            ...
            as_u64(reference.at("identity_bytes"),
                   "checkpoint bounded line component identity bytes") == 0)
          throw std::invalid_argument(
              "checkpoint bounded line component provenance reference is malformed");
      } else if (required_string(
                     component, "provenance_identity").empty()) {
        throw std::invalid_argument(
            "checkpoint line aggregate component provenance is malformed");
      }
    }
...
json::array checkpoint_line_identity_manifest(const json::array& items) {
  ...
  for (const auto& value : items) {
    ...
    const auto& source = (schema == "diffexp3-retained-line-aggregate-v1" ||
                          schema == "diffexp3-retained-line-aggregate-v2" ||
                          schema ==
