...
RecurrenceProblem<Rational> exponential_problem(std::uint32_t nmax) {
  ...
  p.nhat_lags[0].valuations = {diffexp::kernel::kCompleteInfinity};
...
void test_reusable_dense_factorization() {
  ...
  const auto factorization =
      diffexp::kernel::detail::factor_dense_system(matrix);
  const auto first = diffexp::kernel::detail::solve_factored_dense_system(
      factorization, {Rational(7), Rational(3), Rational(5)});
  const auto second = diffexp::kernel::detail::solve_factored_dense_system(
...
void test_lower_frame_guard() {
  ...
  } catch (const diffexp::kernel::RecurrenceError& error) {
    loud = error.id == "E4";
  }
...
RecurrenceProblem<Rational> resonant_jordan_source_problem(
  ...
  p.nhat_lags[0].valuations.assign(4, diffexp::kernel::kCompleteInfinity);
  p.d0_inverse_scalar = Rational(1);
  p.blocks = {JordanBlock{{0, 1}}};
  p.schedule = {{{StepCase::Resonant, Rational(0), Rational(0)}}};

  diffexp::kernel::SourceData<Rational> source;
  source.present = {1, 1};
...
PreparedMatrix<Rational> identity_prepared_matrix(std::uint32_t dimension) {
  ...
  matrix.valuations.assign(
      ...
      diffexp::kernel::kCompleteInfinity);
  MatrixShift<Rational> identity;
  identity.shift = 0;
...
RecurrenceProblem<Rational> pseudo_jordan_problem(
  ...
  problem.nhat_lags[0].valuations.assign(
      4, diffexp::kernel::kCompleteInfinity);
...
void test_json_error_contract() {
  const auto value = boost::json::parse(diffexp::kernel::run_recurrence_json("{}"));
...
void test_malformed_tensor_is_typed_error() {
  ...
  const auto value = boost::json::parse(diffexp::kernel::run_recurrence_json(request));
...
boost::json::object json_request(const std::string& request) {
  return boost::json::parse(diffexp::kernel::run_recurrence_json(request)).as_object();
}
