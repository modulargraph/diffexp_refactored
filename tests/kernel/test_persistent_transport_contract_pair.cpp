...
void rolling_stream_contracts_before_state_publication() {
  const std::string checkpoint =
      "/tmp/diffexp3-rolling-transport-contract-pair.de2cp";
  ...
  try {
    ...
    for (const auto* side : {"lower_tiles", "upper_tiles"}) {
      ...
      if (compact.at("schema") !=
              "diffexp3-compact-rolling-tile-diagnostic-v1" ||
    ...
    const auto rolling_container = diffexp::kernel::checkpoint::read(checkpoint);
    const auto rolling_payload =
        json::parse(rolling_container.payload_json).as_object();
    const auto& rolling_lines =
        rolling_payload.at("retained_line_results").as_array();
    if (rolling_lines.size() != 1)
      throw std::runtime_error(
          "rolling checkpoint lost its single combined line");
    const auto& rolling_record = rolling_lines.front().as_object();
    if (rolling_record.at("schema") !=
        "diffexp3-retained-line-aggregate-v2")
      throw std::runtime_error(
          "rolling checkpoint did not use bounded aggregate provenance");
    const auto& rolling_provenance =
        rolling_record.at("provenance").as_object();
    if (rolling_provenance.at("schema") !=
        "diffexp3-retained-native-line-aggregate-v2")
      throw std::runtime_error(
          "rolling checkpoint lost its bounded native aggregate schema");
    const auto& rolling_locals = rolling_provenance.at("source")
        .as_object().at("locals").as_array();
    if (rolling_locals.empty() || rolling_locals.size() > 2)
      throw std::runtime_error(
          "rolling checkpoint lost its deduplicated terminal local owners");
    for (const auto& raw_local : rolling_locals) {
      const auto& local = raw_local.as_object();
      if (local.if_contains("source_operator_identity") != nullptr ||
          local.if_contains("analytic_metadata") != nullptr ||
          local.if_contains("retained_derivation") != nullptr ||
          local.if_contains("source_operator_reference") == nullptr ||
          local.if_contains("analytic_metadata_reference") == nullptr ||
          local.if_contains("retained_derivation_reference") == nullptr)
        throw std::runtime_error(
            "rolling checkpoint recursively embedded terminal-local provenance");
    }
    const auto& rolling_components = rolling_provenance.at("aggregate")
        .as_object().at("components").as_array();
    if (rolling_components.size() != 2)
      throw std::runtime_error(
          "rolling checkpoint lost its signed arm components");
    for (const auto& raw_component : rolling_components) {
      const auto& component = raw_component.as_object();
      if (component.size() != 4 ||
          component.if_contains("provenance_identity") != nullptr ||
          component.if_contains("provenance_reference") == nullptr)
        throw std::runtime_error(
            "rolling checkpoint recursively embedded arm provenance");
    }
    (void)request(json::object{
...
int main() {
  const std::string checkpoint =
      "/tmp/diffexp3-transport-contract-pair.de2cp";
  const std::string checkpoint_second =
      "/tmp/diffexp3-transport-contract-pair-second.de2cp";
  const std::string checkpoint_tampered =
      "/tmp/diffexp3-transport-contract-pair-tampered.de2cp";
  const std::string checkpoint_count_tampered =
      "/tmp/diffexp3-transport-contract-pair-count-tampered.de2cp";
  const std::string checkpoint_legacy =
      "/tmp/diffexp3-transport-contract-pair-legacy.de2cp";
  std::remove(checkpoint.c_str());
  ...
  try {
    ...
    for (const auto& raw : payload.at("retained_line_results").as_array()) {
      ...
      for (const auto* side_name : {"lower", "upper"}) {
        ...
        if (row.size() != 4 || row.if_contains("tile") == nullptr ||
            row.if_contains("exact_identity_reference") == nullptr ||
            row.if_contains("entry_facts_reference") == nullptr ||
            row.at("entry_count") != 1 ||
            row.if_contains("prepared_row") != nullptr)
          throw std::runtime_error(
              "paired checkpoint retained a deep prepared-row copy");
        for (const auto* reference_name :
             {"exact_identity_reference", "entry_facts_reference"}) {
          const auto& reference = row.at(reference_name).as_object();
          if (reference.at("algorithm") != "fnv1a64-v1" ||
              reference.at("identity_bytes").as_int64() <= 0)
            throw std::runtime_error(
                "paired checkpoint lost a compact row provenance reference");
        }
        if (row.if_contains("exact_identity") != nullptr ||
            row.if_contains("entries") != nullptr)
          throw std::runtime_error(
              "paired checkpoint recursively embedded exact row identities");
      }
    }
