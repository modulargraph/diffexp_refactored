#include "diffexp/kernel/local_algebra.hpp"
...
using diffexp::kernel::PreparedRationalAnalyticCoefficient;
using diffexp::kernel::PreparedRationalTaylorMultiplier;
using diffexp::kernel::PreparedSparseLocalMultiplierMatrix;
...
LocalSolution<Rational> sample() {
  ...
  out.chart.radius = diffexp::kernel::ComplexBall::from_strings("2");
  out.epsilon = {-1, 1};
  ...
  diffexp::kernel::validate_local_solution(out, false);
  return out;
}
...
LocalSolution<ComplexBall> widen_acb_taylor_with_zeros(
  ...
  diffexp::kernel::validate_local_solution(output, false);
  return output;
}
...
LocalSolution<Rational> epsilon_shift_components(
  ...
  for (std::uint32_t component = 0;
    ...
    auto selected =
        diffexp::kernel::local_algebra_detail::with_selected_component(
            source, component);
    if (shifts[component] != 0) {
      ...
      selected = diffexp::kernel::multiply_prepared_rational(
          selected, monomial,
          checkpoint + ":component:" +
              std::to_string(component));
    }
    embedded.push_back(
        diffexp::kernel::local_algebra_detail::embedded_component(
            selected, component, source.dimension));
  }
  return diffexp::kernel::combine_local_solutions(
      embedded, checkpoint);
}
...
int main() {
  ...
  const auto casep_product = diffexp::kernel::multiply_prepared_rational(
  ...
  const auto applied = diffexp::kernel::apply_prepared_sparse_local_matrix(
      matrix, input);
  ...
  const auto unshifted = diffexp::kernel::multiply_prepared_rational(
      input, multiplier(0, 0));
  const auto combined = diffexp::kernel::combine_local_solutions(
  ...
  const auto exact_right_column = [](
    ...
    diffexp::kernel::validate_local_solution(local, false);
    return local;
  };
  ...
  const auto exact_right_transformed =
      diffexp::kernel::right_transform_local_basis_exact(
  ...
  const auto stable_exact_right =
      diffexp::kernel::materialize_local_basis_weights(
  ...
  diffexp::kernel::validate_local_solution(direct_t_source, false);
  ...
  diffexp::kernel::FiniteLaurentMatrix<Rational> bounded_r{
  ...
  const auto confluent_evaluation =
      diffexp::kernel::evaluate_local_solution(
  ...
  try {
    (void)diffexp::kernel::restrict_local_epsilon_frame_strict_lower(
  ...
  empty.rows = 2;
  empty.columns = 2;
  ok = ok && !diffexp::kernel::apply_prepared_sparse_local_matrix(
  ...
  const auto scalar_full = diffexp::kernel::apply_prepared_sparse_local_matrix(
      scalar_row, input, "scalar-full");
  const auto scalar_direct = diffexp::kernel::apply_prepared_scalar_row_window(
  ...
  if (scalar_full.has_value() && scalar_direct.has_value()) {
    const auto scalar_restricted =
        diffexp::kernel::restrict_local_epsilon_frame_strict_lower(
  ...
  const auto zero_guard_full =
      diffexp::kernel::apply_prepared_sparse_local_matrix(zero_guard_row, input);
  const auto zero_guard_direct =
      diffexp::kernel::apply_prepared_scalar_row_window(
  ...
  const auto sparse_full = diffexp::kernel::apply_prepared_sparse_local_matrix(
  ...
  const auto acb_full = diffexp::kernel::apply_prepared_sparse_local_matrix(
      acb_row, acb_input, "acb-full");
  const auto acb_direct = diffexp::kernel::apply_prepared_scalar_row_window(
  ...
  ok = ok && !diffexp::kernel::apply_prepared_scalar_row_window(
  ...
  const auto specialized_negative =
      diffexp::kernel::local_algebra_detail::
  ...
  const auto exact_physicalized =
      diffexp::kernel::apply_prepared_sparse_local_matrix(
          exact_tail_gauge, input,
          "terminal-tail-exact-physicalized");
  const auto specialized_tail_gauge =
      diffexp::kernel::specialize_prepared_rational_matrix_to_acb(
          exact_tail_gauge);
  ...
  try {
    diffexp::kernel::restrict_factorized_local_basis_taylor_prefix(
