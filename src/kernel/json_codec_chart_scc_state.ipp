...
class CompositeSCCChartBase : public PhysicalEquationOwnerBase {
 public:
  ...
  json::object diagnostic_page(const std::size_t page_offset,
                               const std::size_t page_size) const {
    auto result = stats_json();
    auto* raw_blocks = result.if_contains("block_charts");
    if (raw_blocks == nullptr || !raw_blocks->is_array())
      throw std::logic_error(
          "composite SCC statistics lost their block diagnostics");
    const auto& all_blocks = raw_blocks->as_array();
    const auto total = all_blocks.size();
    const auto begin = std::min(total, page_offset);
    const auto end = begin + std::min(total - begin, page_size);
    json::array selected;
    selected.reserve(end - begin);
    for (auto index = begin; index < end; ++index) {
      auto record = all_blocks[index].as_object();
      compact_large_identity(record, "principal_identity");
      selected.emplace_back(std::move(record));
    }
    result["block_charts"] = std::move(selected);
    compact_large_identity(result, "identity");
    compact_large_identity(result, "rational_shadow_identity");
    compact_large_identity(result, "physical_payload_identity");
    const auto has_more = end < total;
    result["diagnostics_page"] = json::object{
        {"offset", page_offset},
        {"size", page_size},
        {"returned_through", end},
        {"total", total},
        {"has_more", has_more},
        {"next_offset",
         has_more ? json::value(end) : json::value(nullptr)}};
    result["diagnostic_detail"] = "paged-scc-statistics";
    return result;
  }
  json::object publication_summary() const {
    auto result = stats_json();
    // Full per-block diagnostics are available through the paged
    // scc.stats operation.  Publishing them again from scc.prepare makes a
    // hot opaque-handle response scale with exact symbolic payload size.
    result.erase("block_charts");
    compact_large_identity(result, "identity");
    compact_large_identity(result, "rational_shadow_identity");
    compact_large_identity(result, "physical_payload_identity");
    result["diagnostic_detail"] = "bounded-scc-publication";
    return result;
  }
 ...
 private:
  static void compact_large_identity(json::object& object,
                                     const char* key) {
    const auto* value = object.if_contains(key);
    if (value == nullptr || !value->is_string()) return;
    const auto& text = value->as_string();
    constexpr std::size_t kInlineIdentityBytes = 64U * 1024U;
    if (text.size() <= kInlineIdentityBytes) return;
    const auto identity = std::string_view(text.data(), text.size());
    object[std::string(key) + "_reference"] =
...
CompositeGaugeTransform<Scalar> parse_composite_gauge_transform(
  ...
  if (required_string(object, "schema") !=
          "diffexp3-scc-gauge-transform-v1" ||
  ...
  if (required_string(proof, "schema") !=
          "diffexp3-scc-gauge-transform-identity-v1" ||
...
parse_composite_spectral_source_transform(
  ...
  if (required_string(object, "schema") !=
      "diffexp3-scc-spectral-source-transform-v1")
    throw std::invalid_argument(
  ...
  if (required_string(producer_record, "schema") !=
          "diffexp3-scc-spectral-source-transform-identity-v1" ||
...
ExactJordanIndicialCertificate parse_exact_jordan_indicial_record(
  ...
  if (required_string(object, "schema") !=
      "diffexp3-exact-affine-jordan-indicial-v1")
    throw std::invalid_argument(
  ...
  const LocalSolution<Scalar>& homogeneous_target(
    ...
    try {
      ...
      const auto contract = json::serialize(canonical_json_value(
          json::object{{"schema", "diffexp3-casep-target-cache-v1"},
                       {"block", block_index_},
                       {"component", component},
                       {"run", run},
    ...
    if constexpr (std::is_same_v<Scalar, ComplexBall>) {
      ...
      json::object column_identity_record{
          {"schema", regular_singular_execution
               ? (scalar_execution
                     ? "diffexp3-native-scc-regular-singular-scalar-column-v1"
                     : "diffexp3-native-scc-regular-singular-jordan-column-v2")
               : (scalar_execution
                     ? "diffexp3-native-scc-column-v1"
                     : "diffexp3-native-scc-column-v2")},
  ...
  pushforward_acb_matching_residual(
    ...
    if constexpr (!std::is_same_v<Scalar, ComplexBall>) {
      ...
      json::object certificate{
          {"schema", "diffexp3-acb-matching-residual-pushforward-v2"},
 ...
 private:
  ...
  std::optional<std::string> acb_matching_normal_frame_identity(
    ...
    return json::serialize(canonical_json_value(json::object{
        {"schema", "diffexp3-acb-scc-matching-normal-frame-v2"},
        {"equation_operator_identity", exact_identity_},
        {"receiving_local_point",
         json::object{{"exact", point.exact_coordinate},
                      {"sign", point.sign}}},
  ...
  json::object certify_column_uniqueness_path(
    ...
    return json::object{
        {"schema", "diffexp3-scc-column-uniqueness-path-v1"},
        {"scc_exact_identity", exact_identity_},
        {"canonical_seed",
  ...
  std::optional<json::object> certify_parent_completeness(
    ...
    if constexpr (!std::is_same_v<Scalar, Rational>) {
      ...
      return json::object{
          {"schema", "diffexp3-scc-parent-exact-formal-completeness-v1"},
  ...
  CompositeColumnSolveResult solve_regular_acb_column(
    ...
    json::object column_identity_record{
        {"schema", regular_singular_execution
             ...
             : (scalar_execution
                   ? "diffexp3-native-scc-acb-regular-scalar-column-v1"
                   : "diffexp3-native-scc-acb-regular-column-v2")},
        {"scc_exact_identity", exact_identity_},
        {"basis_index", basis_index},
        {"seed", seed_request},
        {"targets", target_requests},
