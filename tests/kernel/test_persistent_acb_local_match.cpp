...
std::string prepare_taylor_chart(const std::string& session) {
  auto response = request(json::parse(std::string(R"json({
    "schema":2,"op":"chart.prepare","session":")json") + session +
      R"json(","key":"acb-taylor-prefix-chart",
    "identity":"acb-taylor-prefix-chart-v1",
    "analytic":{
      "geometry":{"center_exact":"0","scale_exact":"1",
        "radius_exact":"2","infinite_radius":false,"prescriptions":[]},
      "principal_matrix":[
      ...
      "native_scc_capabilities":{"regular":true,"identity_gauge":true,
        "identity_v":true,"no_pseudo":true}},
    "scc":{"components":[[0],[1]],"structural_edges":[[0,0],[1,1]],
      "condensation_edges":[],"topological_order":[0,1],
      "coupling_depth":0},
    "problem":{"domain":"acb","precision_bits":256,
      "d":2,"fb":0,"w":6,
  ...
  if (response.at("status") != "ok")
    throw std::runtime_error("prepare Taylor chart: " +
...
json::object taylor_run(std::vector<std::string> initial) {
  json::array encoded_initial;
  for (auto& value : initial)
    encoded_initial.push_back(json::string(std::move(value)));
  json::array shifts;
  ...
  return json::object{
      ...
      {"initial", std::move(encoded_initial)},
      {"initial_validity", json::array{5, 5}}, {"source", nullptr},
      {"return_u", false}};
}
...
std::string solve_taylor_local(const std::string& session,
                               const std::string& chart,
                               std::vector<std::string> initial,
                               const std::string& checkpoint) {
  auto response = request(json::object{
      {"schema", 2}, {"op", "local.solve"}, {"session", session},
      {"chart", chart}, {"run", taylor_run(std::move(initial))},
      {"metadata", metadata(checkpoint)}});
  if (response.at("status") != "ok")
    throw std::runtime_error("solve Taylor " + checkpoint + ": " +
                             json::serialize(response));
  return std::string(response.at("local").as_string());
}
...
std::vector<std::string> constant_column(std::string first_component,
                                         std::string second_component) {
  return {std::move(first_component), "0", "0", "0", "0", "0",
          std::move(second_component), "0", "0", "0", "0", "0"};
}

json::object taylor_match_request(
  ...
  return json::object{
      ...
      {"basis", json::array{first, second}}, {"incoming", incoming},
      {"basis_chart", chart}, {"incoming_chart", chart},
      {"basis_point", json::object{{"exact", "1/3"}}},
      {"incoming_point", json::object{{"exact", "1/3"}}},
      {"epsilon", json::object{{"min", 0}, {"max", 5},
                                {"required_complete_max", 3}}},
      {"basis_checkpoint_identities",
       json::array{"taylor-basis-0", "taylor-basis-1"}},
      {"incoming_checkpoint_identity", "taylor-incoming"},
      {"checkpoint_identity", "acb-stable-taylor-prefix-v1"},
      {"native_unit_saturation",
       json::object{
           {"schema",
            "diffexp3-native-acb-unit-leading-saturation-request-v2"},
           {"tile_plan", "test-taylor-prefix-plan"},
           {"tile_plan_checkpoint_identity",
            "test-taylor-prefix-plan-checkpoint"},
           {"arm", "lower"}, {"match", 0}}},
      {"refinement", json::object{{"relative_tolerance", "1e-100"},
                                    {"max_steps", 0}}}};
}
...
int main() {
  ...
  const auto taylor_chart = prepare_taylor_chart(session);
  const auto taylor_first = solve_taylor_local(
      session, taylor_chart, constant_column("1", "0"),
      "taylor-basis-0");
  const auto taylor_second = solve_taylor_local(
      session, taylor_chart, constant_column("0", "1"),
      "taylor-basis-1");
  const auto taylor_incoming = solve_taylor_local(
      session, taylor_chart, constant_column("3", "2"),
      "taylor-incoming");
  const auto taylor_matched = request(taylor_match_request(
      session, taylor_chart, taylor_first, taylor_second,
      taylor_incoming));
  const auto taylor_match = taylor_matched.at("status") == "ok"
      ? std::string(taylor_matched.at("match").as_string())
      : std::string();
  for (const auto& local : {taylor_first, taylor_second, taylor_incoming})
    (void)request(json::object{{"schema", 2}, {"op", "local.release"},
                               {"session", session}, {"local", local}});
  ...
  const auto checkpoint_path =
      ...
      ("diffexp3-live-acb-match-" + std::to_string(stamp) + ".chk");
  ...
  const auto& taylor_residual = taylor_matched.at("status") == "ok"
      ...
      : json::object{};
  const auto& taylor_basis = taylor_matched.at("status") == "ok"
      ? taylor_matched.at("basis").as_array()
      : json::array{};
  const auto matching_taylor_width =
      taylor_basis.empty()
      ? std::uint64_t{0}
      : (taylor_basis.front().as_object().at("matching_taylor_width")
                 .is_uint64()
             ? taylor_basis.front().as_object()
                   .at("matching_taylor_width").as_uint64()
             : static_cast<std::uint64_t>(
                   taylor_basis.front().as_object()
                       .at("matching_taylor_width").as_int64()));
  const bool stable_taylor_prefix_selected =
      taylor_matched.at("status") == "ok" &&
      taylor_residual.at("verdict") == "pass" &&
      taylor_residual.at("complete_through_required") == true &&
      !taylor_basis.empty() &&
      matching_taylor_width < 5;
  ...
  const auto released_again = request(json::object{
      {"schema", 2}, {"op", "match.release"}, {"session", session},
      {"match", match}});
  const auto taylor_released = taylor_match.empty()
