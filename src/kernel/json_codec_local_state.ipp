...
bool is_retained_plan_value_handoff_schema(std::string_view schema) {
  return schema == "diffexp3-retained-plan-value-handoff-v1" ||
         schema == "diffexp3-retained-plan-value-handoff-v2";
}
...
parse_prepared_physical_ode(const json::value& raw,
  ...
  if (required_string(object, "schema") !=
          "diffexp3-physical-cleared-ode-v1" ||
...
json::object checkpoint_local_analytic_metadata_record(
  ...
  return json::object{
      {"schema", "diffexp3-exact-local-analytic-metadata-v2"},
      {"chart", json::object{
  ...
  for (const auto& prescription : model.prescriptions)
    prescriptions.push_back(json::object{
        ...
        {"leading_coefficient_sign",
         prescription.leading_coefficient_sign}});
  return json::object{
      {"schema", "diffexp3-regular-taylor-tail-model-v1"},
...
RegularTaylorTailModel parse_checkpoint_regular_tail_model(
  ...
  if (required_string(object, "schema") !=
      "diffexp3-regular-taylor-tail-model-v1")
    throw std::invalid_argument(
...
struct OwnerBoundResidualBinding {
  ...
  json::object public_reference() const {
    ...
    return json::object{
        {"schema", "diffexp3-owner-bound-residual-reference-v1"},
...
OwnerBoundResidualBinding make_owner_bound_residual_binding(
  ...
  const json::object source_provenance{
      {"schema", "diffexp3-retained-homogeneous-zero-source-v1"},
  ...
  json::object provenance{
      {"schema", "diffexp3-owner-bound-physical-residual-binding-v2"},
...
class StoredLocalBase {
 public:
  ...
  virtual json::object release_terminal_factorized_owner() = 0;
 ...
 public:
    ...
    if constexpr (std::is_same_v<Scalar, SymbolicRational>) {
      ...
      json::object record{
        {"schema", sealed_plan_match_lineage_
             ? "diffexp3-retained-local-v5"
             : "diffexp3-retained-local-v4"},
        {"handle", handle_},
  ...
  json::object seal_plan_match_lineage() override {
    ...
    if (value_handoff) {
      ...
      sealed = json::object{
          {"schema", "diffexp3-sealed-plan-value-handoff-lineage-v1"},
          {"local", handle_},
      ...
      sealed = json::object{
        {"schema", "diffexp3-sealed-plan-match-local-lineage-v2"},
        {"local", handle_},
        {"local_checkpoint_identity", solution_.checkpoint_identity},
        {"source_operator_identity", source_operator_identity_},
        {"match", derivation.at("source_match")},
        ...
        {"schema", "diffexp3-sealed-plan-match-local-lineage-v1"},
        {"local", handle_},
        {"local_checkpoint_identity", solution_.checkpoint_identity},
        {"source_operator_identity", source_operator_identity_},
        {"match", derivation.at("source_match")},
  ...
  json::object release_terminal_factorized_owner() override {
    if (!sealed_plan_match_lineage_ || retained_owner_ != nullptr)
      throw std::logic_error(
          "terminal factorized owner release requires one sealed owner-free plan-match lineage");
    if (terminal_factorized_owner_ == nullptr ||
        terminal_factorized_owner_checkpoint_identity_.empty())
      throw std::invalid_argument(
          "terminal factorized owner is absent or already released");
    const auto checkpoint_identity =
        terminal_factorized_owner_checkpoint_identity_;
    const auto owners_before = static_cast<std::uint64_t>(
        terminal_factorized_owner_.use_count());
    terminal_factorized_owner_.reset();
    terminal_factorized_owner_checkpoint_identity_.clear();
    return json::object{
        {"released_terminal_factorized_owner", true},
        {"released_owner_checkpoint_identity", checkpoint_identity},
        {"released_owner_shared_owners_before", owners_before}};
  }
