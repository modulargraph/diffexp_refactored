...
SavePersistentCheckpoint::usage = "SavePersistentCheckpoint[owner, path, identity] atomically writes an opaque versioned native checkpoint for prepared charts/SCCs, retained Rational or Acb locals, exact-rational or exact-lattice-guided Acb matches, endpoint results, exact tile plans, and completed line results. Dependency-only locals/plans/charts remain strongly owned without becoming public after restore; symbolic locals remain a loud deferred kind.";
RestorePersistentCheckpoint::usage = "RestorePersistentCheckpoint[path, expectedIdentity] validates and restores an opaque native checkpoint into a new persistent C++ session without replaying Wolfram preprocessing. It returns the restored session plus stable chart, SCC, local, exact/Acb-match, endpoint, tile-plan, and line-result handle maps.";
