...
bool acb_full_rank_pivot_smoke() {
  diffexp::kernel::ComplexBall::set_precision(256);
  using Matrix =
      diffexp::kernel::matching_detail::DenseScalarMatrix<diffexp::kernel::ComplexBall>;
  const auto uncertain = [] {
    return diffexp::kernel::ComplexBall::from_strings("[0 +/- 1]");
  };
  const Matrix triangular{{diffexp::kernel::ComplexBall(1), uncertain()},
                          {diffexp::kernel::ComplexBall(0),
                           diffexp::kernel::ComplexBall(1)}};
  if (diffexp::kernel::matching_detail::certify_full_rank_by_nonzero_pivots(
          triangular, "parallel-arm triangular Acb rank proof") != 2)
    return false;

  const Matrix ambiguous{{uncertain(), diffexp::kernel::ComplexBall(0)},
                         {diffexp::kernel::ComplexBall(0),
                          diffexp::kernel::ComplexBall(1)}};
  try {
    (void)diffexp::kernel::matching_detail::certify_full_rank_by_nonzero_pivots(
        ambiguous, "parallel-arm ambiguous Acb rank proof");
  } catch (const diffexp::kernel::MatchingArithmeticError& error) {
    return error.code == diffexp::kernel::MatchingArithmeticErrorCode::AmbiguousZero;
  }
  return false;
}
...
bool acb_unit_leading_proof_smoke() {
  const std::string checkpoint_path =
      "/tmp/diffexp3-persistent-parallel-arms-acb.de2cp";
...
int main() {
  const std::string checkpoint_path =
      "/tmp/diffexp3-persistent-parallel-arms.de2cp";
  std::remove(checkpoint_path.c_str());
  std::string session;
  std::string restored_session;
  try {
