...
bool environment_flag_is_one(const char* name) {
...
std::string run_recurrence_json(std::string_view input) {
  try {
    ...
    if (const auto* schema = root.if_contains("schema");
      ...
      auto encoded = json::serialize(run_session_command(root));
      protocol::enforce_response_budget(operation, encoded.size());
      return encoded;
    }
