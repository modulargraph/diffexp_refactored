  ...
  if (operation == "transport.consume_hop") {
    ...
    json::object response;
    try {
      const auto planned_hop_identity = json::serialize(
          canonical_json_value(match->handoff()));
      const auto sealed_lineage_identity = json::serialize(
          canonical_json_value(sealed_lineage));
      json::object match_reference{
          {"schema", "diffexp3-bounded-consuming-hop-reference-v1"},
          {"index", match_index},
          {"checkpoint_identity", match->checkpoint_identity()},
          {"provenance_reference",
           compact_matching_identity_reference(
               match->provenance_identity())},
          {"planned_hop_reference",
           compact_matching_identity_reference(planned_hop_identity)},
          {"sealed_local_lineage_reference",
           compact_matching_identity_reference(sealed_lineage_identity)},
          {"exact_lineage_retained", true}};
      ...
      response = json::object{
          {"status", "ok"}, {"session", session->handle},
          {"capability", "consuming-transport-hop-v2"},
          {"native_retained", true}, {"json_coefficients", 0},
          {"arm", arm_name}, {"match", match_index},
          {"next_local", std::move(next_summary)},
  ...
  if (operation == "transport.diagnostic_rebuild_terminal_match") {
    ...
    if (diagnostics == nullptr || std::string(diagnostics) != "1")
      throw std::invalid_argument(
          ...
          "DIFFEXP_DIAGNOSTIC_TERMINAL_STATE=1");
  ...
  if (operation == "transport.contract_pair_stream_finish") {
    ...
    // Row provenance is content-addressed while the full mathematical row is
    // in flight, so the complete retained line identity remains bounded and
    // can be published intact for checkpoint-manifest compatibility.
    published_line["provenance_identity"] = line->provenance_identity();
    output_lines.push_back(std::move(published_line));
          ...
          for (std::size_t tile = 0; tile < input.row_count(); ++tile) {
            input.projected_local_handles.push_back(
                "private:" + arm_checkpoint_identity(
                                 ...
                                 "projected", tile + 1));
            pair_row_records[side].push_back(
                compact_prepared_row_provenance_record(
                    tile, input.row(tile)));
          }
          input.aggregate_record = json::object{
              {"kind", "transport-state-observable-arm"},
