...
using diffexp::kernel::ScalarTraits;
using diffexp::kernel::StepCase;
using diffexp::kernel::StoredLineIntegrationOptions;
...
Fixture<Scalar> exponential_fixture(
  ...
  prepared.nhat_lags[0].valuations = {diffexp::kernel::kCompleteInfinity};
  ...
  problem.initial_validity = {diffexp::kernel::kCompleteInfinity};
  problem.return_u = true;

  auto recurrence = diffexp::kernel::RecurrenceSolver<Scalar>(
      problem, prepared).run();
  auto assembled = diffexp::kernel::assemble_recurrence(
...
void test_rational_vertical_slice() {
  ...
  auto built = diffexp::kernel::prepare_regular_homogeneous_tail_model(
  ...
  auto evaluated = diffexp::kernel::evaluate_local_solution_with_certified_tail(
  ...
  auto integrated =
      diffexp::kernel::integrate_regular_local_line_with_certified_tail(
...
void test_acb_and_loud_noncertification() {
  ...
  auto acb_model = diffexp::kernel::prepare_regular_homogeneous_tail_model(
  ...
  if (acb_certified) {
    const auto evaluated =
        diffexp::kernel::evaluate_local_solution_with_certified_tail(
  ...
  auto zero_model = diffexp::kernel::prepare_regular_homogeneous_tail_model(
  ...
  const auto disk = zero_model.model.has_value()
      ? diffexp::kernel::certify_regular_taylor_disk(*zero_model.model, "1")
      : diffexp::kernel::RegularTaylorDiskCertificate{};
  ...
  auto regulated_model =
      diffexp::kernel::prepare_regular_homogeneous_tail_model(
  ...
  sourced.problem.source = diffexp::kernel::SourceData<Rational>{};
  auto sourced_model = diffexp::kernel::prepare_regular_homogeneous_tail_model(
...
void test_finite_epsilon_physical_tail_model() {
  diffexp::kernel::PreparedPhysicalClearedODE<ComplexBall> equation;
  equation.dimension = 1;
  diffexp::kernel::ExactEpsilonRational<ComplexBall> q0;
  q0.zero = false;
  q0.valuation = 0;
  ...
  diffexp::kernel::ExactEpsilonRational<ComplexBall> one_plus_epsilon;
  ...
  diffexp::kernel::EpsilonVector initial;
  initial.epsilon = {0, 2};
  initial.dimension = 1;
  initial.coefficients = {
      ComplexBall(1), ComplexBall(0),
      ComplexBall::from_strings("1e100")};
  const auto evolution = diffexp::kernel::evolve_ordinary_center_value(
      equation, initial, 4);
  ...
  for (std::uint32_t taylor = 0; taylor <= 4; ++taylor)
    for (std::int32_t power = 0; power <= 2; ++power)
      sector.coefficients[diffexp::kernel::local_detail::sector_index(
          solution, static_cast<std::size_t>(power), taylor, 0)] =
          evolution.at(taylor).at(power, 0);
  solution.sectors.push_back(std::move(sector));

  auto built =
      diffexp::kernel::prepare_physical_regular_homogeneous_tail_model(
          equation, solution);
  ...
  const auto evaluated =
      diffexp::kernel::evaluate_physical_local_solution_with_certified_tail(
  ...
  const auto independently_rounded_coefficient =
      diffexp::kernel::local_detail::sector_index(
...
void test_finite_causal_q_disk_inverse_certificate() {
  ...
  const auto certified =
      diffexp::kernel::tail_majorant_detail::
  ...
  try {
    (void)diffexp::kernel::tail_majorant_detail::
        weighted_physical_ode_prefix_norm_upper(
            normalized, 3);
  ...
  diffexp::kernel::PhysicalRegularTaylorTailModel causal_solution;
  ...
  const auto causal_disk =
      diffexp::kernel::certify_physical_regular_taylor_disk(
          causal_solution, "1/2");
