...
json::object identity_reference(const std::string_view identity) {
  ...
  std::string fingerprint = "fnv1a64:";
  for (int shift = 60; shift >= 0; shift -= 4)
    fingerprint.push_back(digits[(hash >> shift) & UINT64_C(0xf)]);
  return json::object{{"algorithm", "fnv1a64-v1"},
...
json::object request(json::object value) {
  const auto operation = value.if_contains("op") != nullptr
      ? std::string(value.at("op").as_string()) : std::string("schema-1");
  last_operation = operation;
  try {
    auto response = json::parse(
        diffexp::kernel::run_recurrence_json(json::serialize(value))).as_object();
    if (operation == "session.stats" && response.at("status") != "ok")
      throw std::runtime_error("session.stats response: " +
                               json::serialize(response));
    return response;
  } catch (const std::exception& error) {
    throw std::runtime_error("response handling after " + operation +
                             ": " + error.what());
  }
}
...
json::object singular_spectral_source_transform(
  ...
  const auto producer_identity = json::serialize(json::object{
      {"schema", "diffexp3-scc-spectral-source-transform-identity-v1"},
      ...
      {"entries", json::array{json::object{{"row", 0},
          ...
          {"center_pole_order", 0}}}}});
  return json::object{
      {"schema", "diffexp3-scc-spectral-source-transform-v1"},
      {"rows", 1},
      {"columns", 1},
      {"identity", false},
      {"epsilon_unimodular", true},
...
std::string prepare_constant_singular_scc(const std::string& session,
  ...
  json::object manifest{
      ...
      {"blocks",
       json::array{json::object{
           ...
           {"exact_affine_jordan_indicial",
            json::object{
                {"schema", "diffexp3-exact-affine-jordan-indicial-v1"},
                {"dimension", 1},
      ...
      {"rational_shadow_singular_tail",
       json::object{{"schema", "diffexp3-scc-singular-tail-frame-v1"},
...
std::string solve_singular_column(const std::string& session,
  ...
  if (solved.at("execution_capability") != expected_capability)
    throw std::runtime_error("singular SCC lacks scalar capability: " +
                             json::serialize(solved));
  if (json::serialize(solved).size() > 1024U * 1024U ||
      solved.if_contains("metadata") != nullptr ||
      solved.if_contains("source_operator_identity") != nullptr ||
      solved.if_contains("tail_majorant") != nullptr)
    throw std::runtime_error(
        "singular SCC solve exposed an unbounded retained-owner summary: " +
        json::serialize(solved));
  return std::string(solved.at("local").as_string());
}
...
json::object run_arms(const std::string& session, const std::string& plan,
  ...
  return request(json::object{
      ...
      {"checkpoint_policy",
       json::object{{"schema", "diffexp3-deterministic-arm-checkpoints-v1"},
...
bool tamper_singular_proof_session(const std::string& source,
  ...
  auto container = diffexp::kernel::checkpoint::read(source);
...
bool compact_column_reference(const json::object& reference) {
  ...
  return reference.size() == 6 &&
      reference.at("schema") ==
          "diffexp3-retained-scc-column-reference-v1" &&
...
bool compact_singular_proof(const json::object& proof) {
  ...
  for (const auto& raw_source : proof.at("basis").as_array()) {
    ...
    if (source.at("schema") !=
            "diffexp3-retained-match-source-reference-v1" ||
        source.at("authority") !=
...
bool singular_match_published_with_complete_physical_residual(
  ...
  for (const auto& raw_hop :
    ...
    if (native.at("schema") != "diffexp3-retained-acb-match-v6" ||
...
int main() {
  ...
  const std::string checkpoint = "/tmp/diffexp3-singular-scc-arms.de2cp";
  const std::string checkpoint_second =
      "/tmp/diffexp3-singular-scc-arms-second.de2cp";
  const std::string tampered = "/tmp/diffexp3-singular-scc-arms-tampered.de2cp";
  const std::string terminal_normal_checkpoint =
      "/tmp/diffexp3-singular-scc-terminal-normal.de2cp";
  const std::string correlated_seed_checkpoint =
      "/tmp/diffexp3-singular-scc-correlated-seed.de2cp";
  ...
  try {
    ...
    const auto imported_terminal = request(json::object{
        ...
        {"rational_shadow_identity_reference",
         identity_reference("singular-arm-constant-shadow-v1")},
    ...
    if (planned.at("lower_matches") != 1 ||
        ...
        planned.at("lower").as_object().at("match_count") != 1 ||
        planned.at("upper").as_object().at("match_count") != 1)
    ...
    if (before_rejection.at("status") != "ok")
      throw std::runtime_error("initial session.stats: " +
                               json::serialize(before_rejection));
    ...
    const auto before_one_contract = request(json::object{
        {"schema", 2}, {"op", "session.stats"}, {"session", session}});
    std::ostringstream disabled_terminal_diagnostics;
    if (::setenv("DIFFEXP_DIAGNOSTIC_TERMINAL_STATE", "0", 1) != 0)
      throw std::runtime_error(
          "could not set disabled terminal diagnostic environment");
    auto* previous_stderr =
        std::cerr.rdbuf(disabled_terminal_diagnostics.rdbuf());
    json::object one_contract;
    try {
      one_contract = contract_observables(
          session, transport_state, transport_checkpoint,
    ...
    std::cerr.rdbuf(previous_stderr);
    ::unsetenv("DIFFEXP_DIAGNOSTIC_TERMINAL_STATE");
    if (disabled_terminal_diagnostics.str().find(
            "terminal-adjoint-window") != std::string::npos ||
        disabled_terminal_diagnostics.str().find(
            "terminal-adjoint-radius-source") != std::string::npos)
      throw std::runtime_error(
          "DIFFEXP_DIAGNOSTIC_TERMINAL_STATE=0 enabled terminal diagnostics: " +
          disabled_terminal_diagnostics.str());
    ...
    const auto saved_payload = json::parse(
        diffexp::kernel::checkpoint::read(checkpoint).payload_json).as_object();
    ...
    const auto failed_after = last_operation;
    if (!session.empty())
      (void)request(json::object{{"schema", 2}, {"op", "session.close"},
                                 {"session", session}});
    ...
    std::remove(terminal_normal_checkpoint.c_str());
    std::remove(correlated_seed_checkpoint.c_str());
    std::cerr << "FAIL: Acb singular-SCC whole-arm proof after "
              << failed_after << ": " << error.what() << '\n';
    return EXIT_FAILURE;
  }
}
