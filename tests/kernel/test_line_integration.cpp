#include "diffexp/kernel/line_integration.hpp"
...
using diffexp::kernel::EpsilonVector;
...
using FusedWorkItem =
    diffexp::kernel::line_integration_detail::FusedMonomialWorkItem;
...
template <typename Scalar>
bool same_integral_values(const diffexp::kernel::StoredLineIntegral& left,
                          const diffexp::kernel::StoredLineIntegral& right) {
  (void)sizeof(Scalar);
...
template <typename Scalar>
std::pair<diffexp::kernel::StoredLineIntegral, diffexp::kernel::StoredLineIntegral>
materialized_and_fused(
  ...
  const auto projected = diffexp::kernel::apply_prepared_scalar_row_window(
  ...
  return {diffexp::kernel::integrate_stored_local_line(
              *projected, lower, upper, options),
          diffexp::kernel::integrate_prepared_scalar_row_stored(
...
void grouped_center_and_component_reuse() {
  ...
  const auto result = diffexp::kernel::integrate_stored_local_line(
  ...
  check("line result states its stored-truncation-only scope",
        result.scope == LineIntegrationScope::StoredTruncation &&
            result.value.error.guarantee == diffexp::kernel::ErrorGuarantee::None &&
...
void acb_cancellation_is_exact_only() {
  ...
  try {
    (void)diffexp::kernel::integrate_stored_local_line(
        solution, RealEvaluationPoint::rational("0"),
        RealEvaluationPoint::rational("1/2"), options);
  ...
  const auto exact = diffexp::kernel::integrate_stored_local_line(
      solution, RealEvaluationPoint::rational("0"),
      RealEvaluationPoint::rational("1/2"), options);
...
void acb_bounded_divergent_cancellation_is_explicit_and_relative() {
  ...
  options.divergent_cancellation =
      StoredLineIntegrationOptions::BoundedDivergentCancellation{
          diffexp::kernel::Magnitude::decimal("1e-20"), "1e-20",
          "focused-ft-test"};

  const auto accepted = diffexp::kernel::integrate_stored_local_line(
      solution, RealEvaluationPoint::rational("0"),
      RealEvaluationPoint::rational("1/2"), options);
...
void stored_branch_selection() {
  ...
  const auto expected = diffexp::kernel::integrate_sector_monomial(
      SectorMonomialTag::rational("-1/2", "0", 0), lower, upper,
      expected_options);
  bool conflict_rejected = false;
  options.imaginary_sign = 1;
  try {
    (void)diffexp::kernel::integrate_stored_local_line(solution, lower, upper,
                                                options);
  } catch (const NativeIntegrationError& error) {
    conflict_rejected =
...
void input_error_envelope_is_not_discarded() {
  ...
  solution.error.guarantee = diffexp::kernel::ErrorGuarantee::Advisory;
  solution.error.absolute = {diffexp::kernel::Magnitude::one()};
...
void fused_row_uses_exact_rational_endpoint_order() {
  ...
  const auto pentagon = diffexp::kernel::integrate_prepared_scalar_row_stored(
  ...
  try {
    (void)diffexp::kernel::integrate_prepared_scalar_row_stored(
...
void factorized_ordinary_row_applies_center_ball_once() {
  ...
  const auto factorized =
      diffexp::kernel::integrate_ordinary_center_stored_row_factorized(
...
void fused_row_charges_upper_halo_only_to_regulated_center_primitive() {
  ...
  const auto clipped = diffexp::kernel::integrate_prepared_scalar_row_stored(
      row, source, 2, RealEvaluationPoint::rational("0"),
  ...
  source.sectors = {
      sector<Rational>("0", "0", 0,
                       {Rational(1), Rational(0), Rational(0)})};
  const auto ordinary = diffexp::kernel::integrate_prepared_scalar_row_stored(
      row, source, 2, RealEvaluationPoint::rational("0"),
      RealEvaluationPoint::rational("1/2"), observable);
...
void fused_acb_row_keeps_preprojection_cancellation_scale() {
  ...
  const auto fused = diffexp::kernel::integrate_prepared_scalar_row_stored(
      row, source, 0, RealEvaluationPoint::rational("0"),
      RealEvaluationPoint::rational("1/2"), options);
