...
int main() {
  try {
    ...
    const auto divided = diffexp::kernel::evolve_ordinary_center_value(
    ...
    const auto shifted = diffexp::kernel::evolve_ordinary_center_value(
    ...
    noncausal.c_lags[1].push_back(
        PhysicalODEMatrixEntry<Rational>{0, 0, rational(-1, {"1"})});
    const auto rejected = diffexp::kernel::evolve_ordinary_center_value(
    ...
    try {
      (void)diffexp::kernel::evolve_ordinary_center_value(
    ...
    const auto recentered =
        diffexp::kernel::recenter_physical_cleared_ode(
    ...
    const auto singular_recentered =
        diffexp::kernel::recenter_singular_physical_cleared_ode_to_ordinary(
    ...
    diffexp::kernel::ChartGeometry interval_chart;
    ...
    const auto negative_half =
        diffexp::kernel::RealEvaluationPoint::rational("-1/2");
    ...
    const auto factorized_interval =
        diffexp::kernel::evaluate_ordinary_center_value_factorized(
    ...
    // zero series.
    diffexp::kernel::FactorizedOrdinaryCenterEvaluation sparse_transfer;
    ...
    const diffexp::kernel::EpsilonFrame<ComplexBall> sparse_adjoint(
        {-3, 4},
    ...
    const auto sparse_contraction =
        diffexp::kernel::contract_factorized_ordinary_center_adjoint(
