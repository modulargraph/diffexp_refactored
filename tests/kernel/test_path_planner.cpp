#include "diffexp/kernel/path_planner.hpp"
...
using diffexp::kernel::ExactAffineChart;
using diffexp::kernel::ExactArmRequest;
using diffexp::kernel::ExactBranchSheet;
using diffexp::kernel::ExactComplexProjection;
using diffexp::kernel::ExactMatchKind;
using diffexp::kernel::ExactPathPlanOptions;
using diffexp::kernel::ExactPathPlanningError;
using diffexp::kernel::ExactPathTopology;
...
void classic_exact_chain_and_reverse() {
  ...
  ExactArmRequest forward{
      ...
      topology()};
  const auto plan = diffexp::kernel::plan_exact_arm(forward, options);
  ...
  ExactArmRequest reverse{
      ...
      topology()};
  const auto reversed = diffexp::kernel::plan_exact_arm(reverse, options);
...
void box_bubble_singular_endpoint_handoffs() {
  ...
  ExactArmRequest upper{Rational("11/23"), Rational(1),
                        ...
                        upper_topology};
  const auto arms = diffexp::kernel::plan_exact_independent_arms(lower, upper);
  ...
  check("both singular handoff coordinates stay within 1/k",
        diffexp::kernel::exact_path_detail::abs(lower_match.producing_local) <=
                Rational("1/3") &&
            diffexp::kernel::exact_path_detail::abs(lower_match.receiving_local) <=
                Rational("1/3") &&
            diffexp::kernel::exact_path_detail::abs(upper_match.producing_local) <=
                Rational("1/3") &&
            diffexp::kernel::exact_path_detail::abs(upper_match.receiving_local) <=
                Rational("1/3"));
  ...
  try {
    (void)diffexp::kernel::plan_exact_arm(beyond_half);
  ...
  auto rewrite_match = [](diffexp::kernel::ExactArmPlan& plan,
    ...
    match.producing_local =
        diffexp::kernel::exact_path_detail::local_coordinate(left, physical);
    match.receiving_local =
        diffexp::kernel::exact_path_detail::local_coordinate(right, physical);
  ...
  auto rejects_plan = [](const diffexp::kernel::ExactArmPlan& plan) {
    try {
      diffexp::kernel::validate_exact_arm_plan(plan);
      return false;
    } catch (const ExactPathPlanningError&) {
      return true;
    }
...
void singular_true_radius_does_not_override_affine_conditioning() {
  ...
  bool rejected = false;
  try {
    (void)diffexp::kernel::plan_exact_arm(unsafe, options);
  } catch (const ExactPathPlanningError&) {
    rejected = true;
  }
  ...
  ExactArmRequest conditioned{
      ...
      {ExactAffineChart{"conditioned-pre-endpoint", Rational("999/1000"),
       ...
       endpoint},
      endpoint_topology};
  const auto plan = diffexp::kernel::plan_exact_arm(conditioned, options);
  const auto& match = plan.matches.front();
  check("a closer regular chart gives a singular handoff within both 1/k frames",
        diffexp::kernel::exact_path_detail::abs(match.producing_local) <=
                Rational("1/4") &&
            diffexp::kernel::exact_path_detail::abs(match.receiving_local) <=
                Rational("1/4"));
}
...
void forbidden_match_and_independent_arms() {
  ...
  ExactArmRequest avoided{
      ...
      {ExactAffineChart{"avoid-left", Rational(0), Rational(2), Rational(1),
       ...
       ExactAffineChart{"avoid-right", Rational(1), Rational(2), Rational(1),
                        false}},
      forbidden_topology};
  const auto avoided_plan = diffexp::kernel::plan_exact_arm(avoided);
  ...
  bool rejected = false;
  try {
    (void)diffexp::kernel::plan_exact_arm(avoided);
  } catch (const ExactPathPlanningError&) {
    rejected = true;
  }
...
void asymmetric_receiver_clearance() {
  ...
  ExactArmRequest request{
      ...
      ExactPathTopology{}};
  const auto plan = diffexp::kernel::plan_exact_arm(request, options);
  const auto& match = plan.matches.front();
